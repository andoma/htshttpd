		 ...
		 const char *disposition)
{
  ...
  htsbuf_qprintf(&hdrs, "Server: httpbench\r\n");
...
http_serve_requests(http_connection_t *hc, htsbuf_queue_t *spill)
{
  ...
  do {
    ...
    if(tcp_read_line(hc->hc_fd, cmdline, sizeof(cmdline), spill) < 0) {
      fprintf(stderr, "Unable to read cmd\n");
      return;
    }
    fprintf(stderr, "INPUT: %s\n", cmdline);
    if((n = http_tokenize(cmdline, argv, 3, -1)) != 3) {
      fprintf(stderr, "Unable to tookeize command\n");
      return;
    }
	...
	fprintf(stderr, "Unable to header line\n");
	return;
      }
      fprintf(stderr, "INPUT: %s\n", hdrline);

      if(hdrline[0] == 0) {
	fprintf(stderr, "INPUT COMPLETE\n");
    ...
    if(process_request(hc, spill)) {
      fprintf(stderr, "process_request() failed, disconnecting\n");
      break;
    }
